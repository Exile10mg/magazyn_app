...
class MagazynApp:
        ...
        if file_path_excel:
            ...
            try:
                ...
                for row in sheet.iter_rows(min_row=2, max_col=5):
                    nazwa = str(row[0].value)
                    if row[1].value == "tak" and nazwa and nazwa.strip() and nazwa != "TYP PRODUKTU" and nazwa.lower() != "szukaj" and nazwa.lower() != "none":
    ...
    def save_loaded_file(self, file_path, df, col1, col2, source):
        ...
        dane_folder = os.path.join(script_dir, "Dane_Wtryskiwacze")
    ...
    def wczytaj_subiekt(self):
        ...
        if file_path_subiekt:
            ...
            try:
                ...
                df_subiekt = df_subiekt[df_subiekt['Nazwa'].str.contains("WTRYSKIWACZ", case=False)]
